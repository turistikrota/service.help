...
type Repo interface {
	...
	List(ctx context.Context) ([]*Entity, *i18np.Error)
	ListAdmin(ctx context.Context) ([]*Entity, *i18np.Error)
}
...
func (r *repo) List(ctx context.Context) ([]*Entity, *i18np.Error) {
	filters := bson.M{
		fields.IsActive: true,
	}
...
func (r *repo) ListAdmin(ctx context.Context) ([]*Entity, *i18np.Error) {
	return r.helper.GetListFilter(ctx, bson.M{}, r.filterAdminOptions())
}
