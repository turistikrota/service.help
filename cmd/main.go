...
import (
	...
	"github.com/turistikrota/service.help/server/http"
	"github.com/turistikrota/service.help/service"
...
func loadMongo(cnf config.App) *mongo.DB {
	uri := mongo.CalcMongoUri(mongo.UriParams{
		Host:  cnf.DB.Help.Host,
		Port:  cnf.DB.Help.Port,
		User:  cnf.DB.Help.Username,
		Pass:  cnf.DB.Help.Password,
		Db:    cnf.DB.Help.Database,
		Query: cnf.DB.Help.Query,
	})
	d, err := mongo.New(uri, cnf.DB.Help.Database)
	if err != nil {
		panic(err)
	}
	return d
